...
export default function Aside() {
    ...
    function selectConversation(conversation: Conversation) {
        messages.setSelectedConversation(conversation);
    }
    return (
        <div className="flex h-full h-screen max-h-screen w-1/5 flex-col bg-gray-50">
            ...
            <nav className="flex h-5/6 flex-col gap-2 border-t border-gray-200 p-2">
                <button className="flex h-10 items-center gap-2 rounded-md border-b border-gray-200 bg-indigo-600 px-2 text-sm text-white">
                    <img
                        src={plus}
                        alt="plus icon"
                ...
                <div className="flex h-5/6 w-full flex-col gap-3 overflow-y-auto pr-1">
                    {messages.conversations.map((conversation) => {
                        ...
                        return (
                            <button
                                key={conversation.id}
                                className={`flex min-h-[48px] w-full items-center gap-3 overflow-x-hidden rounded-md border border-gray-200 px-2 py-2 text-sm ${btnClass}`}
