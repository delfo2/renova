...
const Message = React.forwardRef<HTMLDivElement, MessageProps>((props, ref) => {
    if (props.message.content === 'loading') {
        return <LoadingMessage ref={ref} />;
    }

    const userMsg = (
        <div ref={ref} className="flex w-full items-start justify-end p-5">
    ...
    const botMsg = (
        <div ref={ref} className="flex w-full items-start justify-start p-5">
            ...
            <div className="flex w-7/12 flex-col gap-1 rounded-r-lg rounded-bl-lg bg-neutral-100 p-2">
                <span className="mb-1 flex items-center gap-1 text-sm text-neutral-700">
                    ...
                    <span>
                        Pensamento de{' '}
                        {(
                            (props.message.completedAt.valueOf() -
                                props.message.createdAt.valueOf()) /
                            1000
                        ).toFixed(0)}
                        s
                    </span>
